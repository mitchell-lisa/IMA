...
- **Data**: Supabase Postgres via a `Repository` interface (`src/lib/server/repo`). `SupabaseRepository` is used when `NEXT_PUBLIC_SUPABASE_URL` and a server key (`SUPABASE_SECRET_KEY`, or legacy `SUPABASE_SERVICE_ROLE_KEY`) are set; otherwise `MemoryRepository`. The browser/auth key is `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` (or legacy `NEXT_PUBLIC_SUPABASE_ANON_KEY`).
