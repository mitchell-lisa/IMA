...
q_heads = ["order", "id", "category", "topic", "branch", "prompt", "help", "weight_logistics_3pl", "weight_light_manufacturing", "weight_other", "critical_atOrBelow", "critical_message"]
sheet("Questions", q_heads, [[n + 1, q["id"], q["category"], q["topic"], q.get("branch", ""), q["prompt"], q.get("help", ""), q["weights"]["logistics_3pl"], q["weights"]["light_manufacturing"], q["weights"]["other"], (q.get("critical") or {}).get("atOrBelow", ""), (q.get("critical") or {}).get("message", "")] for n, q in enumerate(data["questions"])], editable=("topic", "prompt", "help", "critical_message"), widths={"prompt": 60, "help": 50, "critical_message": 50})
...
tests = [
    ["all_documented_reviewed", "logistics_3pl", "every core answer 3", "overall 100, band strong, confidence 100"],
    ["all_undocumented", "logistics_3pl", "every core answer 0", "overall 0, band priority, critical flags present"],
    ["unknowns_reduce_confidence", "logistics_3pl", "all 3 except two answers 'unknown'", "overall 100, confidence 89"],
    ["category_without_data", "logistics_3pl", "claims answers all 'unknown'", "claims score null; overall from other five"],
    ["late_renewal_with_strengths", "logistics_3pl", "all 3 except renewal lead time 0", "finding renewal_starts_late; strengths present"],
    ["business_change_gate", "logistics_3pl", "business changes 0; profile recent change true/false", "finding business_outpaced_governance only when true"],
    ["fleet_branch", "logistics_3pl", "hasVehicles true; fleet controls 0", "branch applicable; critical flag br_fleet_controls"],
    ["contracts_module_focus", "light_manufacturing", "module=contracts; COI 0, requirements 1, subcontractor 1", "focus card lists those three practices"],
]
